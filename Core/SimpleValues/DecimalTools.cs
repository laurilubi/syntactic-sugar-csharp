...
public static class DecimalTools
{
    ...
    public static decimal GetDecimalOrDefault(object input, decimal defaultValue = default)
    {
        var value = GetDecimalOrNull(input);
        return value ?? defaultValue;
    }

    public static decimal GetDecimalOrThrow(object input) => GetDecimalOrThrow(input,
        input => new ModelValidationException($"Value {input} cannot be parsed as decimal."));

    public static decimal GetDecimalOrThrow<TException>(object input, Func<object, TException> createException)
